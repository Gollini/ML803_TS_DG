...
if __name__ == "__main__":
    ...
    parser.add_argument('--train_optimizer', default='sgd', type=str,
        help = "The train_optimizer used for training. Default 'sgd'.")
    ...
    assert train_optimizer in ["sgd", "adadelta", 'rmsprop', "adam", 'adamax', 'nadam']
    ...
    algorithm = algorithm_factory.load_algorithm(config_name=config_name, train_optimizer=train_optimizer, learning_rate=learning_rate)
    ...
    if ("allbutone" in eval_task or "all" in eval_task and len(ds_keys) > 1):
        ...
        results_folder_allbutoneds = os.path.join(folder, "..", "evaluation_output", "evaluation_allbutone_datasets", pred_target, train_optimizer)
        if issubclass(type(algorithm), DepressionDetectionAlgorithm_DL_erm) and dl_strategy != "direct":
            results_folder_allbutoneds_file = results_folder_allbutoneds + f"/{config_name}_{dl_strategy}_{train_optimizer}_{str(learning_rate)}.pkl"
        else:
            results_folder_allbutoneds_file = results_folder_allbutoneds + f"/{config_name}_{train_optimizer}_{str(learning_rate)}.pkl"
