...
def load_algorithm(config_name:str, train_optimizer:str, learning_rate:float) -> DepressionDetectionAlgorithmBase:
    ...
    # DEEP LEARNING ALGORITHMS
        ...
        algorithm = DepressionDetectionAlgorithm_DL_erm(
            config_name = config_name, train_optimizer=train_optimizer, learning_rate=learning_rate
            )
    elif (config_name.startswith("dl_irm")):
        algorithm = DepressionDetectionAlgorithm_DL_irm(
            config_name = config_name, train_optimizer=train_optimizer, learning_rate=learning_rate
            )
    elif (config_name.startswith("dl_mldg")):
        algorithm = DepressionDetectionAlgorithm_DL_mldg(
            config_name = config_name, train_optimizer=train_optimizer, learning_rate=learning_rate
            )
    elif (config_name.startswith("dl_masf")):
        algorithm = DepressionDetectionAlgorithm_DL_masf(
            config_name = config_name, train_optimizer=train_optimizer, learning_rate=learning_rate
            )
    elif (config_name.startswith("dl_dann")):
        algorithm = DepressionDetectionAlgorithm_DL_dann(
            config_name = config_name, train_optimizer=train_optimizer, learning_rate=learning_rate
            )
    elif (config_name.startswith("dl_csd")):
        algorithm = DepressionDetectionAlgorithm_DL_csd(
            config_name = config_name, train_optimizer=train_optimizer, learning_rate=learning_rate
            )
