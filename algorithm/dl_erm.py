...
class DepressionDetectionClassifier_DL_erm(DepressionDetectionClassifierBase):
    ...
    def __init__(self, config):
        ...
        #     folder_name,
        # )
        self.results_save_folder = os.path.join(path_definitions.TMP_PATH,
            'optimization_tunning', self.optimizer_params["optimizer"], "lr-" + str(self.optimizer_params["learning_rate"]),
            self.config["name"], "input_shape-" + "_".join([str(i) for i in self.model_params["input_shape"]]),
            self.config["training_params"]["eval_task"],
            folder_name,
        )
    ...
    def fit(self, X, y):
        ...
        model_optimizer = network.prep_model_optimizer(self.optimizer_params)
...
class DepressionDetectionAlgorithm_DL_erm(DepressionDetectionAlgorithmBase):
    ...
    def __init__(self, config_dict = None, config_name = "dl_erm_1dcnn", train_optimizer = "sgd", learning_rate = 0.01):
        ...
        # Modification: Add the  to automate optimization selection
